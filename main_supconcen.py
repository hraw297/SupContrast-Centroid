...
def train_supconcen(args):
    ...
    # ============ preparing data ... ============
    transform = DataAugmentation(args.dataset_name, args.input_size)

    if args.dataset_name == 'cifar10':
        dataset = datasets.CIFAR10(root='.', transform=transform, download=True)
    elif args.dataset_name == 'cifar100':
        dataset = datasets.CIFAR100(root='.', transform=transform, download=True)
...
def train_one_epoch(student, teacher, teacher_without_ddp, criterion, data_loader,
    ...
    for it, (images, labels) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        for i, param_group in enumerate(optimizer.param_groups):
            ...
            f_t_all = torch.split(teacher_output, [args.batch_size_per_gpu] * (args.local_crops_number + 2), dim=0)
            f_s_all = torch.split(student_output, [args.batch_size_per_gpu] * (args.local_crops_number + 2), dim=0)
            ...
            if args.use_centroid:
                ...
                mean = torch.zeros(aug_n, embed_dim).cuda()
                # print(labels.shape, embed_dim, t_features.shape, aug_mean_ft.shape, s_features.shape, args.out_dim)
                mean = mean.scatter_reduce(0, labels.contiguous().view(-1, 1).repeat(1, embed_dim), aug_mean_ft,\
